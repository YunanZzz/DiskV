...
static IndexIVFPQ* read_ivfpq(IOReader* f, uint32_t h, int io_flags) {
    ...
    if (ivpq->is_trained) {
        ...
        //     // Read clusters array
        //     ivfpqd->clusters = new size_t[nlist];
        //     for (size_t i = 0; i < nlist; ++i) {
        //         READ1(ivfpqd->clusters[i]);
        //     }

        //     // Read len array
        //     ivfpqd->len = new size_t[nlist];
        //     for (size_t i = 0; i < nlist; ++i) {
        //         READ1(ivfpqd->len[i]);
        //     }
        ...
        //     // Read other members
        //     READ1(ivfpqd->top);
        //     READ1(ivfpqd->estimate_factor);
        //     READ1(ivfpqd->estimate_factor_partial);
        //     READ1(ivfpqd->prune_factor);
        //     READ1(ivfpqd->disk_vector_offset);
        ...
        //     ivfpqd->aligned_cluster_info = new Aligned_Cluster_Info[nlist]; // Allocate array dynamically based on nlist
        //     for (size_t i = 0; i < nlist; ++i) {
...
Index* read_index(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("null")) {
        ...
        // if (IndexIVFPQFastScanDisk* ivfpqfsd = dynamic_cast<IndexIVFPQFastScanDisk*>(ivpq)) {
        //     size_t nlist = ivfpqfsd->nlist;
        //     ivfpqfsd->clusters = new size_t[nlist];
        //     for (size_t i = 0; i < nlist; ++i) {
        //         READ1(ivfpqfsd->clusters[i]);
        //     }

        //     ivfpqfsd->len = new size_t[nlist];
        //     for (size_t i = 0; i < nlist; ++i) {
        //         READ1(ivfpqfsd->len[i]);
        //     }
        //     READ1(ivfpqfsd->top);
        //     READ1(ivfpqfsd->estimate_factor);
        //     READ1(ivfpqfsd->estimate_factor_partial);
        //     READ1(ivfpqfsd->prune_factor);
        //     READ1(ivfpqfsd->disk_vector_offset);
        //     READ1(ivfpqfsd->assign_replicas);
        //     size_t path_length;
        //     READ1(path_length);
        //     ivfpqfsd->disk_path.resize(path_length);
        //     READANDCHECK(&ivfpqfsd->disk_path[0], path_length);
        // }
